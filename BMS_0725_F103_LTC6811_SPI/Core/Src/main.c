...
//ERROR VARIABLES
int8_t cvError = 0,auxError = 0,cfgError = 0;	//hold if an error has occured while reading cell/aux/cfg registers
...
//  MX_SPI2_Init();
  ...
  {
		char msg[64];
		uint8_t *rx = bms_ic[0].config.rx_data;

		wakeup_idle(TOTAL_IC);
		LTC6811_wrcfg(TOTAL_IC, bms_ic);
		cfgError = LTC6811_rdcfg(TOTAL_IC, bms_ic);	// 0 = PEC OK

		sprintf(msg, "cfg:%d %02X %02X %02X %02X %02X %02X\r\n",
		        cfgError, rx[0], rx[1], rx[2], rx[3], rx[4], rx[5]);
