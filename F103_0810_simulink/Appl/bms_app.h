 ...
 *   Detect*:  0 = no open wire, 1 = open flag set OR ADC poll timeout
 *             (timeout: return 1 and flags[] cleared)
 * Actual voltages / flags are in the output arrays.
 *
 * ADC wait timeout lives only in bms_app.c (does not call LTC681x_pollAdc).
 *
 ...
 * @return 0 OK; 1 PEC/timeout/read fault. Voltages are in codes[], not in return.
 *
 ...
 * @return 0 OK; 1 PEC/timeout/read fault. Codes are in codes[], not in return.
 *
 ...
 * @return 0 no open wire; 1 open present or ADC poll timeout (timeout => flags all 0).
 *
